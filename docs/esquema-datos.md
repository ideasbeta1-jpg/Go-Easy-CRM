...
> **Lógica Round Robin:** La asignación automática de leads ordena los agentes con `role = 'agente'` por `last_assigned_at ASC` y selecciona el primero (el que lleva más tiempo sin recibir un lead).
...
Registro de todas las acciones automáticas disparadas por el motor. RLS: ❌ · **180 registros**
...
| `google_config` | `jsonb` | Config de Google. Default: `{}` (Nullable) |
